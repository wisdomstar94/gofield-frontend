...
const PageContents = () => {
  ...
  const nextStepButtonClick = useCallback(() => {
    ...
    const applyUrl = `/exchange-return/${orderItemId}/apply`
    const query = {
      reasonList: reasonList,
    };
    const urlQueryString = getNextRouterQueryToUrlQueryString(query);
    router.push(applyUrl + urlQueryString);
  }, [orderItemId, reasonList, router, setGlobalModalDefaultModalItem]);
      ...
      <WindowSizeContainer>
        ...
        <ContentArticle
          ...
          <List __width="100%" __direction="vertical" __defaultItemMarginBottom="24px">
            {
              enumExchangeReturnReasonListQuery.data?.map((item, index) => {
                return (
                  <ListItem key={index}>
                    <Checkbox
                      __checkMode="single" 
        ...
        <BottomFixedOrRelativeBox __heightToRelative={100}>
          <div className="w-full px-6 pb-6 grid grid-cols-2 gap-2">
            ...
            <div>
              <Button __buttonStyle="white-solid-gray-stroke" __onClick={nextStepButtonClick}>다음단계</Button>
            </div>
          </div>
